...
function PopupsPage() {
  ...
  const add = async () => {
    const { error } = await supabase.from("popups").insert({ name: "Popup nou", trigger_type: "time", trigger_value: 5, is_active: false, content: { title: "Titlu", body: "Mesaj" } });
        ...
        {items.map(p => (
          <div key={p.id} className="rounded-xl border border-border bg-card p-4 space-y-2">
            <div className="grid gap-2 md:grid-cols-4">
              ...
              <select value={p.trigger_type || "time"} onChange={e => update(p.id, { trigger_type: e.target.value })} className="rounded-md border border-border bg-background px-3 py-2 text-sm">
                <option value="time">După timp</option><option value="exit_intent">Exit intent</option><option value="scroll">Scroll %</option><option value="manual">Manual</option>
              </select>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-muted-foreground">Afișări: {p.shown_count || 0} • Conversii: {p.conversion_count || 0}</span>
              </div>
